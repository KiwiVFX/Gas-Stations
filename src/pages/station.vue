<template>
  <q-page>
    ...
    > -->
    <!-- <div class="col-12">{{gasStation}}</div> -->
    ...
    <div v-if="isOpenCheck(gasStation.attributes.is_open)" class="col-12 text-h6 text-green blink">פתוח</div>
    <div v-else class="col-12 text-h6">סגור</div>
    <div v-if="breakAdress(gasStation.attributes.t_ktovet)[0] && breakAdress(gasStation.attributes.t_ktovet)[1]" class="col-12 text-h6">{{breakAdress(gasStation.attributes.t_ktovet)[0]}} {{breakAdress(gasStation.attributes.t_ktovet)[1]}}</div>
    <div v-if="breakAdress(gasStation.attributes.t_ktovet)[2]" class="col-12 text-h6">{{breakAdress(gasStation.attributes.t_ktovet)[2]}}</div>
    <!-- stationID is: {{stationId}}
    gasStation is: {{gasStation}} -->
...
export default defineComponent({
  ...
  //  },
  //    setup() {
  //   const center = { lat: this.gasStation.attributes.lat, lng: this.gasStation.attributes.lon }

  //   return { center }
  // },
  // mounted(){
  //   console.log("station id is: ", this.stationId)
  //   console.log("gasStation is: ", this.gasStation)
...
gasStation() {
     ...
     center(){
      return { lat: this.gasStation.attributes.lat, lng: this.gasStation.attributes.lon }
    },
  },
  methods: {
    isOpenCheck(val) {
...
var newArray = []
  var oneLinerTest = value.split(/(?=רחוב: )|(?=מספר בית: )|(?=שכונה: )/gi).filter(String);
